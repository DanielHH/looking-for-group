...
class DataTest(unittest.TestCase):
    ...
    def create_users(self):
        ...
        self.server.post('/user', headers={'Content-Type':'application/json'},
                         data=json.dumps({'username':'eriny656','password':'password'}))
        self.server.post('/user', headers={'Content-Type':'application/json'},
                         data=json.dumps({'username':'somon123','password':'password'}))

    def login_user(self, username):
        self.server.post('/user/login', headers={'Content-Type':'application/json'},
                         data=json.dumps({'username':'{0}'.format(username),'password':'password'}))
        user = server.User.query.filter_by(username='{0}'.format(username)).first()
    ...
    def test_login_user(self):
        ...
        user = server.User.query.filter_by(username='user').first()
    ...
    def test_delete_message(self):
        ...
        self.server.delete('http://127.0.0.1:5000/messages/{0}'.format(message_one_id), headers={"Authorization":token})
        self.server.delete('http://127.0.0.1:5000/messages/{0}'.format(message_two_id), headers={"Authorization":token})
        rv = self.convert_to_literal(self.server.get('/messages'))
